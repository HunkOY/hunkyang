...
public class DateUtils {
    private final static Integer NUMBER_TEN = 10;
    private final static String SPECIAL_CHAR_DOT = ".";
    private final static String SPECIAL_CHAR_COLON = ":";
    ...
    public static String firstDayInMonth() {
        ...
        Date data = getAnyDay(null,0, 1, 0, 0, 0);
     ...
     * @return
     */
    public static String firstDayInNextMonth(){
        SimpleDateFormat format = new SimpleDateFormat(DATETIME_PATTERN);
        Date data = getAnyDay(null,1, 1, 0, 0, 0);
        return format.format(data);
    }
     ...
     * @return
     */
    public static String firstDayInDesignatedMonth(String monthTime){
        Date date=convertDate(monthTime);
        SimpleDateFormat format = new SimpleDateFormat(DATETIME_PATTERN);
        Date data = getAnyDay(date,0, 1, 0, 0, 0);
        return format.format(data);
    }
     ...
     * @return
     */
    public static String firstDayOfNextMonthInDesignatedMonth(String monthTime){
        Date date=convertDate(monthTime);
        SimpleDateFormat format = new SimpleDateFormat(DATETIME_PATTERN);
        Date data = getAnyDay(date,1, 1, 0, 0, 0);
        return format.format(data);
    }
     ...
     * @return
     */
    private static Date getAnyDay(Date date,int month, int dayOfMonth, int hourOfDay, int minute, int second) {
        Calendar c = Calendar.getInstance();
        if (date!=null){
            c.setTime(date);
        }
    ...
    public static Date convertDate(String date) {
        ...
        if (date.indexOf(SPECIAL_CHAR_COLON) > 0) {
            return DateUtils.convertDate(date, DateUtils.DATETIME_PATTERN);
        } else if (date.indexOf(SPECIAL_CHAR_DOT) > 0) {
     ...
     * Describe:
     *
     * @return return:String 日期 yyyy-MM-dd
     * Date:2014-5-26
     * @author:hezhihong
     */
    ...
    public static int minutesToHour(int minutes) {
        int hour = minutes / MINUTES_OF_AN_HOUR;
        if (minutes % MINUTES_OF_AN_HOUR != 0) {
            hour += 1;
        }
        return hour;
    }
     ...
     * @return 返回最后一天
     */
    public static String getLastDayOfMonth(Object date) {
        Calendar cal = Calendar.getInstance();
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        if (date instanceof String) {
            try {
                cal.setTime(format.parse((String) date));
            } catch (ParseException pe) {
                throw new IllegalArgumentException("date parse error :" + date);
            }
    ...
    public static Date parse(String strDate) throws ParseException {
        return StringUtils.isBlank(strDate) ? null : parse(strDate, DATETIME_PATTERN_CREAT_ORDER);
    }
...
//            throw new Exception(e);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(firstDayInNextMonth());
        System.out.println(firstDayInDesignatedMonth("2019-04-01"));
        System.out.println(firstDayOfNextMonthInDesignatedMonth("2019-04-01"));
    }
}
